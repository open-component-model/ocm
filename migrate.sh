...
pkg="github.com/open-component-model/ocm"
prefix="$pkg/"

build=(components .github Dockerfile flake.nix Makefile .goreleaser.yaml dist/config.yaml README.md)
...
mig()
{
  ...
  for s in api pkg cmds hack examples "${@:3}"; do
...
rename() {
  ...
  if [ ${#files[@]} -ne 1 -o "${files[0]}" != "$1/*" ]; then
    ...
    for s in api pkg cmds hack examples; do
      echo "  adapting $s"
...
for s in pkg/{spiff,dirtree,blobaccess,clisupport,cobrautils,encrypt,errkind,mime,out,refmgmt,iotools,listformat,logging,registrations,runtimefinalizer,semverutils,testutils} pkg/common/{accessio,accessobj,compression}; do
  mig $s api/utils
done

mig pkg/common api/common
...
# ocm extensions
for s in api/ocm/{repositories,accessmethods,blobhandler,attrs,download,actionhandler,digester,labels,resourcetypes}; do
  mig $s api/ocm/extensions
done
...
if [ -n "$MODULE" ]; then
  ...
  for s in api cmds components hack examples docs "${build[@]}" go.mod .golangci.yaml; do
    find "$s" -type f -exec sed -i "s:$pkg:$MODULE:g" {} \;
  done
