...
var _ = Describe("upload", func() {
	...
	// This test is a copy of the above "transfers oci artifact with named handler and string config".
	// The difference: ociRef (attr variable) does not have the repository part ("copy").
	// I.e. the configuration instructs the handler to copy OCI artifacts to the root of the OCI registry (TARGET).
	// Because the test is based on CTF, "//" is expected between the target CTF folder and artifact namespace in the image reference.
	// In case of an OCI registry, just "/" would be expected between the host and the namespace (subject to a separate test case).
	It("transfers oci artifact with named handler and string config without repository", func() {
		ctx := env.OCMContext()
		...
		// prepare upload to target OCI repo
		// attr := ociuploadattr.New(TARGET + grammar.RepositorySeparator + grammar.RepositorySeparator + "copy")
		attr := TARGET + grammar.RepositorySeparator + grammar.RepositorySeparator
		MustBeSuccessful(blobhandler.RegisterHandlerByName(ctx, "ocm/ociArtifacts", attr))
		...
		val := Must(ctx.AccessSpecForSpec(acc))
		// TODO: the result is invalid for ctf: better handling for ctf refs
		Expect(val.(*ociartifact.AccessSpec).ImageReference).To(Equal("/tmp/target//ocm/value:v2.0@sha256:" + D_OCIMANIFEST1))
		...
		Expect(err).To(Succeed())
		defer Close(target)
		Expect(target.ExistsArtifact("ocm/value", "v2.0")).To(BeTrue())
	})
})
