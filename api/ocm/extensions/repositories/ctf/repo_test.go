...
var _ = Describe("access method", func() {
	...
	It("adds component version", func() {
		...
		MustBeSuccessful(cv.SetResourceBlob(meta, blobaccess.ForString(mime.MIME_TEXT, S_TESTDATA), refhints.NONE, nil))
		...
		Expect(cv.SetResourceBlob(meta, blobaccess.ForString(mime.MIME_TEXT, S_TESTDATA), refhints.NONE, nil)).To(MatchError("unable to set resource: digest mismatch: " + D_TESTDATA + " != fake"))
	...
	It("provides error for invalid bloc access", func() {
		...
		// add resource
		Expect(ErrorFrom((cv.SetResourceBlob(compdesc.NewResourceMeta("text1", resourcetypes.PLAIN_TEXT, metav1.LocalRelation), blobaccess.ForFile(mime.MIME_TEXT, "non-existing-file"), refhints.NONE, nil)))).To(MatchError(`file "non-existing-file" not found`))
