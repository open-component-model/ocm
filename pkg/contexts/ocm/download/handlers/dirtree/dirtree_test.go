...
var _ = Describe("artifact management", func() {
	...
	BeforeEach(func() {
		...
		MustBeSuccessful(utils.CreateTarFromFs(Must(projectionfs.New(env, "testdata/layers/0")), "layer0.tgz", env))
		MustBeSuccessful(utils.CreateTarFromFs(Must(projectionfs.New(env, "testdata/layers/1")), "layer1.tgz", env))
