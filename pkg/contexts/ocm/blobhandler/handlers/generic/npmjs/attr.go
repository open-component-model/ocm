...
const (
	ATTR_KEY   = "github.com/open-component-model/ocm/npmjs"
	ATTR_SHORT = "npmjs"
)

func init() {
	err := datacontext.RegisterAttributeType(ATTR_KEY, AttributeType{}, ATTR_SHORT)
	if err != nil {
		return
	}
}
...
func (a AttributeType) Description() string {
	return `
*npmjs repository ref*
Upload local npmjs artifact blobs to a dedicated repository.
`
}

func (a AttributeType) Encode(v interface{}, marshaller runtime.Marshaler) ([]byte, error) {
	if _, ok := v.(*Attribute); !ok {
		return nil, fmt.Errorf("npmjs Upload Attribute structure required")
	}
...
func (a AttributeType) Decode(data []byte, unmarshaler runtime.Unmarshaler) (interface{}, error) {
	var value Attribute
	err := unmarshaler.Unmarshal(data, &value)
	if err == nil {
		if value.Repository != nil {
			if value.Repository.GetType() == "" {
				return nil, errors.ErrInvalidWrap(errors.Newf("missing repository type"), string(data))
			}
			return &value, nil
		}
		if value.Ref == "" {
			return nil, errors.ErrInvalidWrap(errors.Newf("missing repository or ref"), string(data))
		}
		data = []byte(value.Ref)
	}
	ref, err := oci.ParseRef(string(data))
	if err != nil {
		return nil, errors.ErrInvalidWrap(err, string(data))
	}
