...
var _ = Describe("access method", func() {
	...
	It("local setting", func() {
		Expect(compatattr.Get(ctx)).To(BeFalse())
		Expect(compatattr.Set(ctx, true)).To(Succeed())
		Expect(compatattr.Get(ctx)).To(BeTrue())
	})

	It("global setting", func() {
		Expect(compatattr.Get(cfgctx)).To(BeFalse())
		Expect(compatattr.Set(ctx, true)).To(Succeed())
		Expect(compatattr.Get(ctx)).To(BeTrue())
	})

	It("parses string", func() {
		Expect(compatattr.AttributeType{}.Decode([]byte("true"), runtime.DefaultJSONEncoding)).To(BeTrue())
	})
})
