...
func OCIRef(repoCtx *ocireg.OCIRegistryRepositorySpec, name, version string) (string, error) {
...
type Cache interface {
	...
	Get(ctx context.Context, repoCtx *ocireg.OCIRegistryRepositorySpec, name, version string) (*compdesc.ComponentDescriptor, error)
	// Store stores a component descriptor in the cache.
	Store(ctx context.Context, descriptor *compdesc.ComponentDescriptor) error
}
...
type Resolver struct {
	...
	client     Client
	cache      Cache
	decodeOpts []compdesc.DecodeOption
}

// NewResolver creates a new resolver.
func NewResolver(client Client, decodeOpts ...compdesc.DecodeOption) *Resolver {
	return &Resolver{
		log:        logr.Discard(),
		client:     client,
...
func (r *Resolver) Resolve(ctx context.Context, repoCtx *ocireg.OCIRegistryRepositorySpec, name, version string) (*compdesc.ComponentDescriptor, error) {
...
func (r *Resolver) ResolveWithBlobResolver(ctx context.Context, repoCtx *ocireg.OCIRegistryRepositorySpec, name, version string) (*compdesc.ComponentDescriptor, ctf.BlobResolver, error) {
	return r.resolve(ctx, repoCtx, name, version, true)
}

// resolve resolves a component descriptor by name and version within the configured context.
// If withBlobResolver is false the returned blobresolver is always nil
func (r *Resolver) resolve(ctx context.Context, repoCtx *ocireg.OCIRegistryRepositorySpec, name, version string, withBlobResolver bool) (*compdesc.ComponentDescriptor, ctf.BlobResolver, error) {
	repo := repoCtx
	// setup logger
	...
	cd, err := compdesc.Decode(componentDescriptorBytes, r.decodeOpts...)
	if err != nil {
		return nil, nil, fmt.Errorf("unable to decode component descriptor: %w", err)
	}
	if err := cd.AddRepositoryContext(repo); err != nil {
		return nil, nil, err
	}

	if r.cache != nil {
		if err := r.cache.Store(ctx, cd.Copy()); err != nil {
...
func (r *Resolver) fetchManifest(ctx context.Context, repoCtx *ocireg.OCIRegistryRepositorySpec, name, version string) (*ocispecv1.Manifest, string, error) {
...
func NewBlobResolver(client Client, ref string, manifest *ocispecv1.Manifest, cd *compdesc.ComponentDescriptor) ctf.BlobResolver {
	return &blobResolver{
		client:   client,
		ref:      ref,
		manifest: manifest,
