...
import (
	...
	"github.com/gardener/ocm/pkg/oci/client/utils/oci"
	cdv2 "github.com/gardener/ocm/pkg/ocm/compdesc/versions/v2"
)
...
var _ = Describe("helper", func() {
	...
	Context("OCIRef", func() {
		...
		It("should correctly parse a repository url with a protocol and a component", func() {
			repoCtx := ocireg.NewOCIRegistryRepositorySpec("http://example.com", "")
			ref, err := oci.OCIRef(repoCtx, "somecomp", "v0.0.0")
			Expect(err).ToNot(HaveOccurred())
			Expect(ref).To(Equal("example.com/component-descriptors/somecomp:v0.0.0"))
		})
		...
		It("should correctly parse a repository url without a protocol and a port and a component", func() {
			repoCtx := ocireg.NewOCIRegistryRepositorySpec("example.com:443", "")
			ref, err := oci.OCIRef(repoCtx, "somecomp", "v0.0.0")
			Expect(err).ToNot(HaveOccurred())
		...
		It("should correctly parse a repository url with a protocol and a port and a component", func() {
			repoCtx := ocireg.NewOCIRegistryRepositorySpec("http://example.com:443", "")
			ref, err := oci.OCIRef(repoCtx, "somecomp", "v0.0.0")
			Expect(err).ToNot(HaveOccurred())
			Expect(ref).To(Equal("example.com:443/component-descriptors/somecomp:v0.0.0"))
		})
		...
		It("should correctly parse a repository url with a sha256-digest name mapping", func() {
			repoCtx := ocireg.NewOCIRegistryRepositorySpec("example.com:443", ocireg.OCIRegistryDigestMapping)
			ref, err := oci.OCIRef(repoCtx, "somecomp", "v0.0.0")
			Expect(err).ToNot(HaveOccurred())
...
type testCache struct {
	get   func(ctx context.Context, repoCtx *ocireg.OCIRegistryRepositorySpec, name, version string) (*cdv2.ComponentDescriptor, error)
	store func(ctx context.Context, descriptor *cdv2.ComponentDescriptor) error
}

var _ oci.Cache = &testCache{}

func (t testCache) Get(ctx context.Context, repoCtx *ocireg.OCIRegistryRepositorySpec, name, version string) (*cdv2.ComponentDescriptor, error) {
