...
for i in "${build_matrix[@]}"; do
  ...
  -ldflags "-s -w \
            ...
            -X github.com/open-component-model/ocm/api/version.gitTreeState=$([ -z "$(git status --porcelain 2>/dev/null)" ] && echo clean || echo dirty) \
            -X github.com/open-component-model/ocm/api/version.gitCommit=$(git rev-parse --verify HEAD) \
            -X github.com/open-component-model/ocm/api/version.buildDate=$(date -u +%FT%T%z)" \
