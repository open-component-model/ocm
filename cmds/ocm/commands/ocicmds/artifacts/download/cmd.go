...
func (d *download) Add(e any) error {
...
func (d *download) Save(o *artifacthdlr.Object, f string) error {
	...
	default:
		...
		// When --oci-layout is specified, store blobs at blobs/<algorithm>/<encoded>
		// per OCI Image Layout Specification instead of blobs/<algorithm>.<encoded>.
		if opts.OCILayout {
			createOpts = append(createOpts, artifactset.OCIBlobLayout(true))
		}
